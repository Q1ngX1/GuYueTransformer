...
from model.cga import CascadedGroupAttentionLayer, CascadedGroupAttentionEncoder
...
class MusicTransformer(nn.Module):
    ...
    def __init__(self, n_layers=6, num_heads=8, d_model=512, dim_feedforward=1024,
        ...
        # RPR Transformer
            ...
            encoder_layer = CascadedGroupAttentionLayer(self.d_model, self.nhead, self.d_ff, self.dropout)
            encoder = CascadedGroupAttentionEncoder(encoder_layer, self.nlayers, encoder_norm)
            self.transformer = nn.Transformer(
                d_model=self.d_model, nhead=self.nhead, num_encoder_layers=self.nlayers,
                num_decoder_layers=0, dropout=self.dropout, # activation=self.ff_activ,
