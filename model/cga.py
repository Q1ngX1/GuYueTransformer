...
def rms_norm(x, dim=-1, eps=1e-8):
    return torch.sqrt((x ** 2).mean(dim=dim, keepdim=True) + eps)


# Cascaded group attention encoder layer
class CascadedGroupAttentionLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        super(CascadedGroupAttentionLayer, self).__init__()
        self.nhead = nhead
        self.d_model = d_model
        self.attention_layers = nn.ModuleList([
            nn.MultiheadAttention(d_model // nhead, nhead)
            for _ in range(nhead)
        ])
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
        self.residual_layer1 = nn.Linear(d_model, d_model)
        self.residual_layer2 = nn.Linear(d_model, d_model)
    ...
    def forward(self, x):
        ...
        pad_size = (self.nhead - (seq_len % self.nhead)) % self.nhead
        padding = torch.zeros(batch_size, pad_size, self.d_model, device=x.device) if pad_size > 0 else torch.zeros(0)
        x = torch.cat([x, padding], dim=1)

        # Create a mask to prevent attention beyond the current position
        mask = torch.triu(torch.ones((1, seq_len + pad_size, seq_len + pad_size), device=x.device), diagonal=1)

        splits = torch.split(x, seq_len // self.nhead, dim=1)
        outputs = []
        for i in range(self.nhead):
            split = splits[i]

            # Update mask for the current head
            head_mask = mask[:, :, :split.size(1)] if pad_size > 0 else mask

            attn_output, _ = self.attention_layers[i](split, split, split, attn_mask=head_mask)
            attn_output = self.rms_norm(self.dropout(attn_output))
            residual = self.residual_layer1(attn_output)
            output = self.norm1(residual + split)

            outputs.append(output)

        concatenated_output = torch.cat(outputs, dim=-1)
        if pad_size > 0:
            concatenated_output = concatenated_output[:, :seq_len, :]

        residual_output = self.residual_layer2(self.feedforward(concatenated_output)) + concatenated_output
        rms_output = self.rms_norm(self.dropout(residual_output),dim=-1)
        output = self.norm2(self.dropout(rms_output+concatenated_output))

        return output

#Cascaded group attention encoder
class CascadedGroupAttentionEncoder(nn.Module):
    def __init__(self, d_model, num_layers=6, dropout=0.1):
        super(CascadedGroupAttentionEncoder, self).__init__()
        self.layers = nn.ModuleList([
            CascadedGroupAttentionLayer(d_model, nhead=8, dim_feedforward=2048, dropout=dropout)
